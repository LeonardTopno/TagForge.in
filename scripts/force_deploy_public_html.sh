...
cp -f "$ROOT/assets/css/app.css" "$DEST/assets/css/app.css"
cp -f "$ROOT/assets/js/api.js" "$DEST/assets/js/api.js"
cp -f "$ROOT/assets/js/app.js" "$DEST/assets/js/app.js"
cp -f "$ROOT/assets/js/admin.js" "$DEST/assets/js/admin.js"
[[ -f "$ROOT/assets/js/barcode.js" ]] && cp -f "$ROOT/assets/js/barcode.js" "$DEST/assets/js/barcode.js"
cp -RF "$ROOT/assets/." "$DEST/assets/" || true
cp -RF "$ROOT/scripts/." "$DEST/scripts/" || true
[[ -f "$ROOT/.htaccess" ]] && cp -f "$ROOT/.htaccess" "$DEST/.htaccess"
touch "$DEST/api.php" "$DEST/includes/auth_guards.php" "$DEST/includes/platform.php" "$DEST/includes/helpers.php" "$DEST/includes/schema.php" "$DEST/includes/layout.php" "$DEST/assets/js/app.js" "$DEST/assets/css/app.css"
...
test -f "$DEST/assets/js/app.js" && echo "app.js: OK" || echo "app.js: MISSING"
grep -q "information_schema.COLUMNS" "$DEST/includes/schema.php" && echo "column-check fix: OK" || echo "column-check fix: MISSING"
grep -q "app-ready" "$DEST/assets/js/app.js" && echo "app-ready: OK" || echo "app-ready: MISSING"
grep -q "boot-splash" "$DEST/includes/layout.php" && echo "boot-splash: OK" || echo "boot-splash: MISSING"
...
ls -la "$DEST/assets/js/app.js" "$DEST/assets/css/app.css" "$DEST/includes/layout.php"
